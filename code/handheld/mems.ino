...
#if USE_SD
  memsFile = SD.open(MEMS_FILE_NAME, WRITE_MODE);  // Create or open a file called "data.txt" on the SD card
  if (memsFile) {
#if USE_DATE
    printDate(memsFile);
    //Sample Nuber
    ...
    #if USE_SERIAL
    Serial.println("Problem Writing MEMS data to the SD card.");
    #endif
  }
#endif
